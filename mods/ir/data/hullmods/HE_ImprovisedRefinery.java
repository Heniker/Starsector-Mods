...
public class HE_ImprovisedRefinery extends BaseLogisticsHullMod {
  ...
  public float getRecievedMetals(float usedOre) {
    return MyMisc.round(usedOre * BASE_CONVERSION_RATIO * CONVERSION_TAX, 2);
  }
  ...
  public String getDescriptionParam(int index, ShipAPI.HullSize hullSize, ShipAPI ship) {
    ...
    if (index == 1) {
      try {
        ...
        return "" + (int) getRecievedMetals(cargo, 100);
      } catch (NullPointerException err) {
        return "" + (int) getRecievedMetals(100);
      }
    }
